        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {modules.map((mod, i) => (
            <button key={mod.title}
              ...
              className={`text-left rounded-2xl p-6 relative overflow-hidden group transition-all duration-500 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              style={{
                background: hoveredModule === mod.title ? mod.grad : "rgba(255,255,255,0.02)",
                border: `1px solid ${hoveredModule === mod.title ? mod.glow + "60" : "rgba(255,255,255,0.06)"}`,
                boxShadow: hoveredModule === mod.title ? `0 0 60px ${mod.glow}30, 0 20px 40px rgba(0,0,0,0.4)` : "none",
                transform: hoveredModule === mod.title ? "translateY(-4px) scale(1.02)" : "translateY(0) scale(1)",
              ...
              <div className="text-4xl mb-3 group-hover:scale-110 transition-transform duration-300 inline-block"
                style={{ filter: hoveredModule === mod.title ? `drop-shadow(0 0 12px ${mod.glow})` : "none" }}>
                {mod.icon}
              </div>

              {/* Title */}
              <h3 className="font-black text-white text-base mb-1.5 group-hover:text-white transition-colors">
                {mod.title}
              </h3>

              {/* Description */}
              <p className="text-gray-600 text-xs leading-relaxed group-hover:text-gray-400 transition-colors">
                {mod.description}
              </p>
