  ...
  return (
    <main className="min-h-screen text-white px-6 py-10" style={{ background: "linear-gradient(135deg, #0f0f0f 0%, #1a0533 50%, #0f0f0f 100%)" }}>
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-2"><span className="text-4xl">✍️</span><h1 className="text-3xl font-bold">Content & Copywriting</h1></div>
          ...
          <div>
            ...
            <div className="flex gap-3 flex-wrap">
              {["landing", "pricing", "feature", "about", "product"].map((t) => (
                <button key={t} onClick={() => setPageType(t)} className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-all ${pageType === t ? "bg-purple-600 text-white shadow-lg shadow-purple-900" : "bg-gray-800 bg-opacity-60 text-gray-300 hover:bg-gray-700"}`}>{t}</button>
              ))}
            </div>
          </div>
          <div>
            <label className="text-sm text-purple-400 mb-2 block font-medium">Key Benefit to Highlight</label>
            <input type="text" value={benefit} onChange={(e) => setBenefit(e.target.value)} placeholder="e.g. Save 10 hours a week on content creation" className="w-full border border-gray-700 rounded-xl px-4 py-3 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-purple-500 transition-colors" style={{ background: "rgba(17, 24, 39, 0.8)" }} />
          </div>
          <div>
            <label className="text-sm text-purple-400 mb-2 block font-medium">Target Persona</label>
            <input type="text" value={persona} onChange={(e) => setPersona(e.target.value)} placeholder="e.g. SaaS founders, marketing managers" className="w-full border border-gray-700 rounded-xl px-4 py-3 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-purple-500 transition-colors" style={{ background: "rgba(17, 24, 39, 0.8)" }} />
          </div>
          <div>
        ...
        {result && (
          <div className="rounded-2xl p-6 border border-purple-900 border-opacity-30" style={{ background: "rgba(26, 5, 51, 0.4)", backdropFilter: "blur(10px)" }}>
            <div className="flex justify-between items-center mb-4">
              ...
              <ExportButtons content={result} filename="content-copy" productName={persona} />
            </div>
